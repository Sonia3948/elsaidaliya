...
// CreateSupplierSubscriptionNotification creates a notification for supplier subscription
func CreateSupplierSubscriptionNotification(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	supplierID, err := primitive.ObjectIDFromHex(data.SupplierID)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "SupplierID invalide"})
		return
	}
	...
	notification := models.Notification{
		UserID:      adminID,
		FromID:      supplierID,
		FromName:    data.SupplierName,
		Type:        models.NotificationTypeOffer,
		Title:       "Demande d'abonnement fournisseur",
