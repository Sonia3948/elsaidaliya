...
// InitNotificationHandlersFunc initializes the notification handlers
func InitNotificationHandlersFunc(db *mongo.Database) {
	notificationCollection = db.Collection("notifications")
}

// CreateNotificationFunc creates a new notification
func CreateNotificationFunc(c *gin.Context) {
	...
	result, err := notificationCollection.InsertOne(ctx, notif)
	if err != nil {
...
// CreatePaymentNotificationFunc creates a payment notification
func CreatePaymentNotificationFunc(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
...
// GetUserNotificationsFunc retrieves notifications for a user
func GetUserNotificationsFunc(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	cursor, err := notificationCollection.Find(ctx, bson.M{"userID": userID}, findOptions)
	if err != nil {
...
// GetAdminNotificationsFunc retrieves notifications for admin
func GetAdminNotificationsFunc(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	cursor, err := notificationCollection.Find(ctx, bson.M{"userID": adminID}, findOptions)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": "Erreur lors de la récupération des notifications"})
		return
	}
...
// MarkNotificationAsReadFunc marks a notification as read
func MarkNotificationAsReadFunc(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
	...
	result, err := notificationCollection.UpdateOne(ctx, filter, update)
	if err != nil {
...
// UpdateNotificationStatusFunc updates the status of a notification
func UpdateNotificationStatusFunc(c *gin.Context) {
	ctx, cancel := context.WithTimeout(context.Background(), 10*time.Second)
	defer cancel()
