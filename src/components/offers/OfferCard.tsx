...
const OfferCard: React.FC<OfferCardProps> = ({
  ...
  return (
    <Card className={`overflow-hidden ${
      highlighted
        ? "border-pharmacy-dark border-2"
        : "border-gray-200"
    }`}>
      {highlighted && (
        <div className="bg-pharmacy-dark py-1 px-4 text-center">
          <p className="text-white text-sm font-medium">Recommandé</p>
        </div>
      )}
      ...
      <CardFooter>
        <Link to={`/payment/${offerId}`} className="w-full">
          <Button 
            className={`w-full ${
              highlighted 
                ? "bg-pharmacy-dark hover:bg-pharmacy" 
                : "bg-pharmacy hover:bg-pharmacy-dark"
            }`}
          >
            Choisir cette offre
          </Button>
